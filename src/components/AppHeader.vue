<template>
  <nav>
    <ul id="menu">
      <li><img src="/static/images/pixy.png"/></li>
      <li class="right"><form v-on:submit.prevent="search">
	<input v-model="keywords" placeholder="Hashtag Search">
	<a href="#" v-on:click="search"><i class="fas fa-search"></i></a>
      </form></li>
      <li class="search" v-if="loggedIn">
	          <router-link :to="{ name: 'UserPage', params: {userID: user.id}}">{{user.username}}</router-link>
            <router-link to="/">Home</router-link>
            <a @click="logout" href="#">Logout</a></p>
      </li>
      <li class="search" v-else>
